...
select * from student left join courses on student.student_id=courses.student_id where courses.student_id is not null; -- left excllusive joins (only get comman data between both remove null values)
select * from student right join courses on student.student_id=courses.student_id where student.student_id is not null; -- right exclusive joins
